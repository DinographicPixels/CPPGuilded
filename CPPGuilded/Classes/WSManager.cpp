...
CPPGuilded::WSManager::WSManager(const std::string& token): threadpool(threadpool), ioc(), ctx{ssl::context::tlsv13_client} {
...
void CPPGuilded::WSManager::startHeartbeating() {
	...
	heartbeat_thread = std::thread([this, taskID] {
		log.registerThread(std::this_thread::get_id(), "Thread-Heartbeat-" + taskID);
	...
	threadpool->execute([this, payload, payloadOPCode]{
		switch(payloadOPCode){
			case GatewayOPCode_Event: {
				...
				ON_GATEWAY_PARSED_PACKET(payloadEventName, payloadData);
				break;
			}
...
void CPPGuilded::WSManager::ON_GATEWAY_PARSED_PACKET(std::string& name, json& data){
...
std::shared_future<void> CPPGuilded::WSManager::send(const json& message) {
	return threadpool->execute([this, message]() {
		if (connected == false && clientReady == true) {
